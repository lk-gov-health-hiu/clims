...
public class PatientRoomController implements Serializable {
    ...
    public List<PatientRoom> getSelectedItems() {
        selectedItems = getFacade().findBySQL("select c from PatientRoom c where c.retired=false and i.patientRoomType = lk.gov.health.data.PatientRoomType.Pharmacy and upper(c.name) like '%" + getSelectText().toUpperCase() + "%' order by c.name");
