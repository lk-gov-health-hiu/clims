...
import lk.gov.health.data.CssFontStyle;
import lk.gov.health.data.CssTextAlign;
import lk.gov.health.data.CssTextDecoration;
...
//            m.put("t", InvestigationItemType.Value);
    ...
    public List<InvestigationItem> completeIxItem(String qry) {
        ...
        if (qry.trim().equals("") || currentInvestigation == null || currentInvestigation.getId() == null) {
            ...
            String sql;
            sql = "select i from InvestigationItem i where i.retired=false and i.ixItemType = lk.gov.health.data.InvestigationItemType.Value and upper(i.name) like '%" + qry.toUpperCase() + "%' and i.item.id = " + currentInvestigation.getId();
    ...
    public List<InvestigationItem> getCurrentIxItems() {
        ...
        if (currentInvestigation == null || currentInvestigation.getId() == null) {
            ...
            String sql;
            sql = "select i from InvestigationItem i where i.retired=false and i.ixItemType = lk.gov.health.data.InvestigationItemType.Value and i.item.id = " + currentInvestigation.getId();
            iivs = getEjbFacade().findBySQL(sql);
        }
        if (iivs == null) {
            iivs = new ArrayList<>();
