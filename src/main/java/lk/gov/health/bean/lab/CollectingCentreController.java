...
public class CollectingCentreController implements Serializable {
    ...
    public List<Institution> completeCollecting(String query) {
        ...
        if (query == null) {
            ...
            sql = "select p from Institution p where p.retired=false and p.institutionType=lk.gov.health.data.InstitutionType.CollectingCentre and upper(p.name) like '%" + query.toUpperCase() + "%' order by p.name";
...
//                + "and i.institutionType = lk.gov.health.data.InstitutionType.CollectingCentre  "
//                + "and upper(c.name) like '%" + getSelectText().toUpperCase() + "%' order by c.name");
    ...
    public List<Institution> getItems() {
        if (items == null) {
            String sql = "SELECT i FROM Institution i where i.retired=false and i.institutionType = lk.gov.health.data.InstitutionType.CollectingCentre order by i.name";
