...
public class InstitutionBranchController implements Serializable {
    ...
    public List<Institution> completeCompany(String qry) {
        String sql;
        sql = "select c from Institution c where c.retired=false and c.institutionType=lk.gov.health.data.InstitutionType.Company and upper(c.name) like '%" + qry.toUpperCase() + "%' order by c.name";
        return getFacade().findBySQL(sql);
    }
