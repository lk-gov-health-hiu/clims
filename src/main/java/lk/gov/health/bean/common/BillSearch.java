...
public class BillSearch implements Serializable {
    ...
    public void fillBillTypeSummery() {
        ...
        if (billClassType == null) {
            j = "select new lk.gov.health.data.BillSummery(b.paymentMethod, sum(b.total), sum(b.discount), sum(b.netTotal), sum(b.vat), count(b), b.billType) "
                    + " from Bill b "
                    + " where b.retired=false "
                    + " and b.billTime between :fd and :td ";
        } else {
            j = "select new lk.gov.health.data.BillSummery(b.paymentMethod, b.billClassType, sum(b.total), sum(b.discount), sum(b.netTotal), sum(b.vat), count(b), b.billType) "
                    + " from Bill b "
                    + " where b.retired=false "
                    + " and b.billTime between :fd and :td ";
        }
    ...
    public void fillBillFeeTypeSummery() {
        ...
        String j;
        if (billClassType == null) {
            j = "select new lk.gov.health.data.BillSummery(b.paymentMethod, sum(bf.feeGrossValue), sum(bf.feeDiscount), sum(bf.feeValue), sum(bf.feeVat), count(b), b.billType) "
                    + " from BillFee bf inner join bf.bill b "
                    + " where b.retired=false "
                    + " and b.billTime between :fd and :td ";
        } else {
            j = "select new lk.gov.health.data.BillSummery(b.paymentMethod, b.billClassType, sum(bf.feeGrossValue), sum(bf.feeDiscount), sum(bf.feeValue), sum(bf.feeVat), count(b), b.billType) "
                    + " from BillFee bf inner join bf.bill b "
                    + " where b.retired=false "
                    + " and b.billTime between :fd and :td ";
        }
