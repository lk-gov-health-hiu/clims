...
public class TheatreController implements Serializable {
    ...
    public List<Department> getSelectedItems() {
        selectedItems = getFacade().findBySQL("select c from Department c where c.retired=false and i.departmentType = lk.gov.health.data.DepartmentType.Theatre and upper(c.name) like '%" + getSelectText().toUpperCase() + "%' order by c.name");
    ...
    public List<Department> getItems() {
        if (items == null) {
            String sql = "SELECT i FROM Department i where i.retired=false and i.departmentType = lk.gov.health.data.DepartmentType.Theatre order by i.name";
