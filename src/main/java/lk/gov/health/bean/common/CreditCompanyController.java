...
public class CreditCompanyController implements Serializable {
    ...
    public List<Institution> completeCredit(String query) {
        ...
        if (query == null) {
            ...
            sql = "select p from Institution p where p.retired=false and p.institutionType=lk.gov.health.data.InstitutionType.CreditCompany and upper(p.name) like '%" + query.toUpperCase() + "%' order by p.name";
    ...
    public List<Institution> getCreditCompanies() {
        String sql;
        sql = "select p from Institution p where p.retired=false and p.institutionType=lk.gov.health.data.InstitutionType.CreditCompany order by p.name";
        return getFacade().findBySQL(sql);
    }

    public List<Institution> getSelectedItems() {
        selectedItems = getFacade().findBySQL("select c from Institution c where c.retired=false and i.institutionType = lk.gov.health.data.InstitutionType.CreditCompany  and upper(c.name) like '%" + getSelectText().toUpperCase() + "%' order by c.name");
    ...
    public List<Institution> getItems() {
        if (items == null) {
            String sql = "SELECT i FROM Institution i where i.retired=false and i.institutionType = lk.gov.health.data.InstitutionType.CreditCompany order by i.name";
