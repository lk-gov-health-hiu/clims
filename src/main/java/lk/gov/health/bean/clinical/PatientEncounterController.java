...
import lk.gov.health.entity.clinical.ClinicalFindingValue;
...
//        selectedItems = getFacade().findBySQL("select c from PatientEncounter c where c.retired=false and i.institutionType = lk.gov.health.data.PatientEncounterType.Agency and upper(c.name) like '%" + getSelectText().toUpperCase() + "%' order by c.name");
