...
public class ManufacturerController implements Serializable {
    ...
    public List<Institution> completeManu(String qry) {
        if (qry != null) {
            institutionList = getFacade().findBySQL("select c from Institution c where c.institutionType=lk.gov.health.data.InstitutionType.Manufacturer and c.retired=false and upper(c.name) like '%" + qry.toUpperCase() + "%' order by c.name");
        }
    ...
    public List<Institution> getItems() {
        if (items == null) {
            String sql = "SELECT i FROM Institution i where i.retired=false and i.institutionType = lk.gov.health.data.InstitutionType.Manufacturer order by i.name";
