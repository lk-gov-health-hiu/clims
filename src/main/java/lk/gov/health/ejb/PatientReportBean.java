...
import lk.gov.health.entity.lab.Antibiotic;
import lk.gov.health.entity.lab.Investigation;
import lk.gov.health.entity.lab.InvestigationItem;
import lk.gov.health.entity.lab.InvestigationItemValueFlag;
...
import lk.gov.health.facade.AntibioticFacade;
import lk.gov.health.facade.InvestigationFacade;
import lk.gov.health.facade.InvestigationItemFacade;
import lk.gov.health.facade.InvestigationItemValueFlagFacade;
...
//    List,
    public List<InvestigationItem> getInvestigationItemsOfValueTypeForInvestigation(Investigation ix) {
        ...
        if (ix == null || ix.getId() == null) {
            ...
            String sql;
            sql = "select ii from InvestigationItem ii where ii.retired = false and ii.ixItemType = lk.gov.health.data.InvestigationItemType.Value and ii.item.id = " + ix.getId() + " order by ii.cssTop, ii.cssLeft ";
        ...
        for (ReportItem ii : temIx.getReportItems()) {
            ...
            if ((ii.getIxItemType() == InvestigationItemType.Value || ii.getIxItemType() == InvestigationItemType.Calculation || ii.getIxItemType() == InvestigationItemType.Flag || ii.getIxItemType() == InvestigationItemType.Template) && ii.isRetired() == false) {
                ...
                if (ptReport.getId() == null || ptReport.getId() == 0) {
                    ...
                    sql = "select i from PatientReportItemValue i where i.patientReport.id = " + ptReport.getId() + " and i.investigationItem.id = " + ii.getId() + " and i.investigationItem.ixItemType = lk.gov.health.data.InvestigationItemType.Value";
